# Surfen

외국인 고객 대상 수상레포츠 송객 사업의 운영 허브.

## 사업 개요

- **모델**: 파트너 사업장(**서프앤 / Surfen**)에 외국인 고객을 송객하고 커미션을 받는 구조
- **담당 영역**: 마케팅 · 세일즈 · 현장 가이드/CS
- **타겟**: 한국 방문 외국인 관광객(영어권 중심)
- **파트너 특징**: 풀라인 수상레포츠 + **제트서프·플라이포일보드 전문** + **협회·대회 운영 권위** — 자세한 사항은 [`01_business/partner-info.md`](01_business/partner-info.md)

## 디렉토리 구조

| 폴더 | 용도 |
| --- | --- |
| `01_business/` | 사업 개요, 파트너 정보, 매출 시나리오, 미팅 어젠다 |
| `02_marketing/` | 페르소나, 채널 전략, 콘텐츠 캘린더, 경쟁 분석 |
| `03_sales/` | 가격, 예약 플로우, 영업 스크립트, OTA 영문 카피 |
| `04_cs/` | 영문 FAQ, 가이드 스크립트, 안전 브리핑, 트러블슈팅 |
| `05_operations/` | 운영 체크리스트, 작업 관리 |
| `docs/` | 회의록, 기타 참고 |

## 핵심 문서 (먼저 보기)

**사업 큰 그림**
- [overview.md](01_business/overview.md) — 한 줄 정의·파트너십·위치
- [partner-info.md](01_business/partner-info.md) — 서프앤 디테일 (브라우저 직접 분석 기반)
- [partner-pricing-confirmed.md](01_business/partner-pricing-confirmed.md) — **공식 가격표 (2026-05-20 확정)**
- [revenue-scenarios.md](01_business/revenue-scenarios.md) — 매출·커미션 시나리오 3안 (실가격 기반)

**전략·실행**
- [registration-checklist.md](02_marketing/registration-checklist.md) — 채널 등록 로드맵
- [competitive-landscape.md](02_marketing/competitive-landscape.md) — 가평 시장 분석
- [content-calendar-30day.md](02_marketing/content-calendar-30day.md) — 첫 30일 콘텐츠
- [free-stay-positioning.md](02_marketing/free-stay-positioning.md) — **"Stay Included" 마케팅 전략 (핵심)**
- [klook-category-keywords.md](02_marketing/klook-category-keywords.md) — Klook 카테고리·키워드
- [ota-listing-copy-en.md](03_sales/ota-listing-copy-en.md) — OTA 영문 카피 즉시 사용

**사장님 미팅**
- [partner-meeting-agenda.md](01_business/partner-meeting-agenda.md) — 60분 미팅 스크립트
- [partner-intake-questions.md](01_business/partner-intake-questions.md) — 인테이크 시트 (한글)
- [서프앤 리서치 - 창수.xlsx](01_business/서프앤%20리서치%20-%20창수.xlsx) — 엑셀 버전 (사장님께 전달)

## 협업

- 모든 문서는 한국어로 작성하되, 고객 응대용 자료(`04_cs/`)는 영문으로 작성합니다.
- 변경사항은 의미 있는 단위로 커밋합니다.
